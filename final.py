...
from utils import load_sentences, get_n_fold_splitting, create_maps, load_embeddings, create_input_tensors, AverageMeter, clip_gradient, calculate_f1_score, save_checkpoint, adjust_learning_rate
...
def main(fold, task, batch_size, epoch):
    ...
    # To suppress the warning of dropout
    config.char_rnn_layers = 2
    config.word_rnn_layers = 2
    ...
    train_words, train_tags, train_pos_mask, val_words, val_tags, val_pos_mask, test_words, test_tags, test_pos_mask = get_n_fold_splitting(sentences, tags, pos_mask, fold, fold_num)
    ...
    if config.checkpoint is not None:
        ...
        word_map, char_map, tag_map, mask_map = create_maps(train_words + val_words, train_tags + val_tags, train_pos_mask+val_pos_mask, config.min_word_freq, config.min_char_freq)  # create word, char, tag maps
        embeddings, word_map, lm_vocab_size = load_embeddings(config.emb_file, word_map, config.expand_vocab)  # load pre-trained embeddings
...
def eval(test_loader, model, crf_criterion, vb_decoder, tag_map, config):
    ...
    for i, (wmaps, cmaps_f, cmaps_b, cmarkers_f, cmarkers_b, tmaps, wmap_lengths, cmap_lengths, pos_mask) in enumerate(
        ...
        # Load word map from pickle file
        ...
        for item in wmaps_sorted:
            sentence = []
            for word in item:
                if word != 0:
                    sentence.append(rev_word_map[word.item()])
            print(' '.join(sentence))
...
if __name__ == '__main__':
    ...
    for f in range(fold_num):
        ...
        true_pos, false_pos, true_neg, false_neg, loc_true_pos, loc_false_pos, loc_true_neg, loc_false_neg, loc_multi_pos = main(f, task, batch_size, epoch)
        b_true_pos += true_pos
        b_false_pos += false_pos
        b_true_neg += true_neg
        b_false_neg += false_neg
